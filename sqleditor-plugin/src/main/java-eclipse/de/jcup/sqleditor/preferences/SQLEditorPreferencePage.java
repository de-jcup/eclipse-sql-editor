...
public class SQLEditorPreferencePage extends FieldEditorPreferencePage implements IWorkbenchPreferencePage {
	...
	protected void createFieldEditors() {
		...
		codeAssistWithSQLKeywords = new BooleanFieldEditor(P_CODE_ASSIST_ADD_KEYWORDS.getId(),
				"SQL keywords", codeAssistGroup);
		codeAssistWithSQLKeywords.getDescriptionControl(codeAssistGroup)
		.setToolTipText("When enabled the standard keywords supported by sql editor are always automatically available as code proposals");
		addField(codeAssistWithSQLKeywords);
	...
	private void setBoolean(SQLEditorPreferenceConstants id, boolean value) {
		getPreferences().setBooleanPreference(id, value);
	}

	private boolean getBoolean(SQLEditorPreferenceConstants id) {
		return getPreferences().getBooleanPreference(id);
	}

	private boolean getDefaultBoolean(SQLEditorPreferenceConstants id) {
